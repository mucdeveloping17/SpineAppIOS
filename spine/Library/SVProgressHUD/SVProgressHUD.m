...
static const CGFloat SVProgressHUDVerticalSpacing = 0.0f;
static const CGFloat SVProgressHUDHorizontalSpacing = 0.0f;
static const CGFloat SVProgressHUDLabelSpacing = 0.0f;
...
+ (void)showInfoImage:(NSTimeInterval)duration {
    [self showImage:[self sharedView].infoImage duration:duration];
//    [self showImage:[self sharedView].infoImage status:nil];
...
+ (void)showImage:(UIImage*)image duration:(NSTimeInterval)duration {
    [[self sharedView] showImage:image status:nil duration:duration];
}
...
- (instancetype)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:frame])) {
        ...
        self.imageViewContainer.alpha = 0.0f;
...
- (void)updateHUDFrame {
    ...
    if(imageUsed || progressUsed) {
        contentWidth = CGRectGetWidth(imageUsed ? self.imageViewContainer.frame : self.indefiniteAnimatedView.frame);
        contentHeight = CGRectGetHeight(imageUsed ? self.imageViewContainer.frame : self.indefiniteAnimatedView.frame);
    }
    ...
    self.imageViewContainer.center = CGPointMake(CGRectGetMidX(self.hudView.bounds), centerY);

    // Label
    if(imageUsed || progressUsed) {
        centerY = CGRectGetMaxY(imageUsed ? self.imageViewContainer.frame : self.indefiniteAnimatedView.frame) + SVProgressHUDLabelSpacing + labelHeight / 2.0f;
    } else {
        centerY = CGRectGetMidY(self.hudView.bounds);
    }
...
- (void)showProgress:(float)progress status:(NSString*)status {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            strongSelf.imageViewContainer.hidden = YES;
...
- (void)showImage:(UIImage*)image status:(NSString*)status duration:(NSTimeInterval)duration {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            strongSelf.imageViewContainer.hidden = NO;
            ...
            if (self.graceTimeInterval > 0.0 && self.backgroundView.alpha == 0.0f) {
                strongSelf.graceTimer = [NSTimer timerWithTimeInterval:self.graceTimeInterval target:strongSelf selector:@selector(fadeIn:) userInfo:((duration > 0) ? @(duration) : nil) repeats:NO];
                [[NSRunLoop mainRunLoop] addTimer:strongSelf.graceTimer forMode:NSRunLoopCommonModes];
            } else {
                [strongSelf fadeIn:((duration > 0) ? @(duration) : nil)];
            }
        }
    }];
}
...
- (UIImageView*)imageView {
    ...
    if(!_imageViewContainer) {
        _imageViewContainer = [[UIImageView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, 97.0f, 97.0f)];
        _imageViewContainer.backgroundColor = [UIColor clearColor];
        [_imageViewContainer.layer setCornerRadius:15];
    }
    
    if(!_imageView.superview) {
        [_imageViewContainer addSubview:_imageView];
        _imageView.center = _imageViewContainer.center;
    }
    
    if(!_imageViewContainer.superview) {
        [self.hudView.contentView addSubview:_imageViewContainer];
    }
...
- (void)fadeInEffects {
    ...
    self.imageViewContainer.alpha = 1.0f;
