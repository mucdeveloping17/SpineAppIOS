...
extension MDPModel {
    ...
    private func isEligible(date: Date?) -> Bool {
        ...
        if let minDate = minDate, let maxDate = maxDate {
            return (minDate...maxDate).contains(date) && setWeekDays(date: date)
        } else if let minDate = minDate {
            return date >= minDate && setWeekDays(date: date)
        } else if let maxDate = maxDate {
            return date <= maxDate && setWeekDays(date: date)
        }
        return true
    }
    
    func setWeekDays(date: Date) -> Bool{
